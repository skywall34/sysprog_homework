...
void create_thread(pthread_t * thread, void * varArguments, int thread_type) {
    int ret;
    if(thread_type == 1) ret = pthread_create(thread, NULL, send_thread, varArguments);
    if(thread_type == 2) ret = pthread_create(thread, NULL, receive_thread, varArguments);
    if (ret == -1) {
...
void * send_thread(void * arg) {
  ...
  send_message(CONNECT_MSG, *queueID, *key, *client_key, *key);
  ...
  while(fgets(buffer, MSGSTR_LEN, stdin)) {
    ...
    if(strcmp(EXIT_STR, input) == 0) {
      send_message(EXIT_STR, *queueID, *key, *client_key, *key);
      exit(0);
    }

    //parsing out the key and message
    if(pos) {
      other_client_key = atoi(strncpy(numBuff, input, pos));
      char * message;
      if(other_client_key) {
        message = buffer+pos+1; // add space index message start
        //send message
        printf("Sending \"%s\" to %d\n", message, other_client_key);
        send_message(message, *queueID, *key, *client_key, other_client_key);
        if(strcmp(message, EXIT_STR) == 0) exit(0); //exit if you say to exit
      }
      else {
        //global send here
        message = buffer;
        send_message(message, *queueID, *key, *client_key, 0);
      }
    }
    else printf(USAGE_STRING);
    //clear the buffers
    strncpy(numBuff, "", 255); //clear buffer
    strncpy(messBuff, "", 255); //clear buffer
  }
...
void * receive_thread(void * arg) {
  ...
  msgbuf messagebuf;
  while(1) {
    receive_message(*queueID, &tempbuf, *client_key);
...
int main(int argc, char * argv[]) {
  ...
  printf("\t***** System Programming Assignment IPC  *****");
  
  varArguments[0] = queueID;
  varArguments[1] = key;
  varArguments[2] = client_key;

  //create threads for sender/receiver
  create_thread(&(threads[0]), varArguments, 1);
  create_thread(&(threads[1]), varArguments, 2);
